...
public class DoubleDoubleConcurrentChainedMapTest extends TestCase
{
    ...
    protected IDoubleDoubleConcurrentMap getMap( final int size, final float ff )
    {
        return DoubleMapFactory.concurrentDoubleDoubleMap( size, ff );
    }
    ...
    private void testPutHelper( final int threads, final float ff ) throws InterruptedException {
        ...
        final IDoubleDoubleConcurrentMap map = getMap( INITIAL_CAPACITY, ff );
    ...
    private static class Adder implements Runnable
    {
        ...
        private final IDoubleDoubleConcurrentMap m_map;

        public Adder( double from, double to, CountDownLatch startGate, CountDownLatch endGate, IDoubleDoubleConcurrentMap map ) {
            m_from = from;
            m_to = to;
        ...
        public void run() {
            try {
                ...
                for ( double n = m_from; n < m_to; ++n ) {
                    assertEquals( ZERO, m_map.put( n, (double) n ) );
    ...
    private static class Updater implements Runnable
    {
        ...
        public Updater( double from, double to, CountDownLatch startGate, CountDownLatch endGate, IDoubleDoubleConcurrentMap map ) {
            m_from = from;
            m_to = to;
            m_startGate = startGate;
            m_endGate = endGate;
    ...
    private static class AddUpdateRemover implements Runnable
    {
        ...
        public AddUpdateRemover( double from, double to, CountDownLatch startGate, CountDownLatch endGate, IDoubleDoubleConcurrentMap map ) {
            m_from = from;
            m_to = to;
            m_startGate = startGate;
            m_endGate = endGate;
        ...
        public void run() {
            try {
                ...
                while ( add < m_to )
                {
                    ...
                    assertEquals( ZERO, m_map.put( add, ONE ) );
                    assertEquals( ONE, m_map.get( add++ ) );
                    final double preRemove = m_map.get( remove );
                    final double removeRes = m_map.remove(remove++);
                    if ( removeRes != ONE )
    ...
    private static class Remover implements Runnable
    {
        ...
        public Remover( double from, double to, CountDownLatch startGate, CountDownLatch endGate, IDoubleDoubleConcurrentMap map ) {
            m_from = from;
            m_to = to;
            m_startGate = startGate;
            m_endGate = endGate;
