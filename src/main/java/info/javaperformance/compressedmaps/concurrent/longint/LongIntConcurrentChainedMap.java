 ...
 * out of it, while it can be updated in the mean time by other thread.
 *
...
public class LongIntConcurrentChainedMap implements IConcurrentLongIntMap
{
     ...
     * @return A new chain and an old value
     */
    private UpdateResult addToChainSlow( final long bucket, final Iterator iter, final Block inputBlock, final int inputStartOffset, final long key, final int value )
    {
    ...
    Special case:
    If bucket length = 1, we do not need to transfer the chain. Instead we can move the {@code long} key right to the new map.

    Once finished iteration, reduce the number of workers. The last one moves new table to the current one.
     */
